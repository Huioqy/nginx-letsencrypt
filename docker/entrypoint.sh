...
if [ -z "${DOMAIN_OR_PUBLIC_IP_2}" ]; then
  printf "\n  =======¡ERROR!======="
  printf "\n  Variable 'DOMAIN_OR_PUBLIC_IP_2' it's necessary\n"
  exit 0
fi
...
printf "\n    - Domain name 2: %s" "${DOMAIN_OR_PUBLIC_IP_2}"
...
sed -i "s/{domain_name_2}/${DOMAIN_OR_PUBLIC_IP_2}/g" /etc/nginx/conf.d/*
